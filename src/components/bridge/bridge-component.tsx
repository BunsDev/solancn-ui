...
import {
  BRIDGE_NETWORKS,
  BRIDGE_PRESET_AMOUNTS,
  BRIDGE_SAMPLE_TOKENS,
  type BridgeNetwork,
  type BridgeToken,
} from "@/lib/constants/bridge";
              ...
              {selectedNetwork && (
                <div className="flex items-center gap-2">
                  ...
                  {selectedNetwork.isTestnet && (
                    <Badge
                      variant="outline"
                      className="ml-1 text-[#14F195] border-[#14F195]/30"
                    >
                      Testnet
                    </Badge>
                  )}
...
export function BridgeComponent() {
  ...
  // States
  const [sourceNetwork, setSourceNetwork] = useState<BridgeNetwork | null>(
    BRIDGE_NETWORKS.solana,
  );
  const [targetNetwork, setTargetNetwork] = useState<BridgeNetwork | null>(
    BRIDGE_NETWORKS.ethereum,
  );
  const [sourceToken, setSourceToken] = useState<BridgeToken | null>(
    BRIDGE_SAMPLE_TOKENS[0],
  );
  const [targetToken, setTargetToken] = useState<BridgeToken | null>(
    BRIDGE_SAMPLE_TOKENS[2],
  );
  ...
  const hasInsufficientBalance = sourceToken && amountNum > sourceToken.balance;
  ...
  return (
    <Card className="bg-background/80 backdrop-blur-sm border border-[#9945FF]/20 shadow-xl w-full max-w-md mx-auto">
                ...
                <div className="flex space-x-2">
                  ...
                  <div className="w-full">
                    ...
                    <div className="flex justify-between mt-1 text-xs text-text/70">
                      <span>
                        Balance: {sourceToken?.balance.toFixed(6) || "0"}
                      </span>
                      <span
                        className={hasInsufficientBalance ? "text-red-500" : ""}
                      >
                  ...
                  <div className="w-full">
                    ...
                    <div className="relative">
                      <Input
                        ...
                        type="text"
                        value={
                          expectedAmount > 0 ? expectedAmount.toFixed(6) : "0"
                        }
                        readOnly
                        ...
                        <Tooltip>
                          ...
                          <TooltipContent className="bg-background border border-[#9945FF]/20 p-2 shadow-md">
                            <div className="text-xs">
                              <p>
                                Network Fee: {feeEstimate.networkFee.toFixed(6)}
                              </p>
                              <p>
                                Bridge Fee: {feeEstimate.bridgeFee.toFixed(6)}
                              </p>
                              <p>
                                Processing Time: ~{feeEstimate.processingTime}
                              </p>
                            </div>
                          </TooltipContent>
                        </Tooltip>
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-text/70 flex items-center">
                            {tx.from} <ArrowRight className="w-3 h-3 mx-1" />{" "}
                            {tx.to}
                          </div>
                        </div>
                      </div>
      ...
      <CardFooter className="flex justify-between pt-0">
        <div className="text-xs text-text/50">Powered by Solana</div>
        <button
          type="button"
          className="text-xs text-[#9945FF] hover:text-[#14F195] transition-colors"
        >
