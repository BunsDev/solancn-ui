...
import {
  mockTradingMarketPairs,
  mockTradingOrderbook,
  mockTradingRecentTrades,
...
export function TradingComponent() {
  ...
  const handleAmountChange = (value: string) => {
    ...
    if (value && price) {
      setTotal(
        (Number.parseFloat(value) * Number.parseFloat(price)).toFixed(2),
      );
    } else {
      setTotal("");
    }
  ...
  const handleTotalChange = (value: string) => {
    ...
    if (value && price) {
      setAmount(
        (Number.parseFloat(value) / Number.parseFloat(price)).toFixed(6),
      );
    } else {
      setAmount("");
    }
          ...
          <CardContent className="p-0">
            ...
            <div className="mt-2 overflow-auto max-h-[300px]">
              <table className="w-full">
                ...
                <tbody>
                  {mockTradingMarketPairs.map((pair) => (
                    <tr
                      key={pair.name}
                    ...
                    >
                      ...
                      <td className="text-right p-4 text-sm">
                        {pair.name.includes("BONK")
                          ? pair.price.toFixed(8)
                          : pair.price.toFixed(2)}
                      </td>
                      <td
                        className={`text-right p-4 text-sm ${pair.change >= 0 ? "text-[#14F195]" : "text-red-500"}`}
                      >
                        {pair.change >= 0 ? "+" : ""}
                        {pair.change}%
                      </td>
            ...
            <CardTitle className="text-[#14F195] text-lg flex justify-between">
              ...
              <div className="flex space-x-2">
                <Button
                  variant="outline"
                  size="sm"
                  className="border-[#9945FF]/30 text-sm hover:bg-[#9945FF]/10"
                >
                  1H
                </Button>
                <Button
                  variant="outline"
                  size="sm"
                  className="border-[#9945FF]/30 text-sm bg-[#9945FF]/20"
                >
                  4H
                </Button>
                <Button
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <div className="space-y-1">
                      {mockTradingOrderbook.asks.map((ask) => (
                        <div
                          key={ask.price}
                  ...
                  <div>
                    ...
                    <div className="space-y-1">
                      {mockTradingOrderbook.bids.map((bid) => (
                        <div
                          key={bid.price}
                          className="flex justify-between text-sm bg-[#14F195]/10 p-2 rounded"
                        >
                ...
                <div className="overflow-auto">
                  <table className="w-full">
                    <thead className="border-b border-[#9945FF]/20">
                      <tr>
                        <th className="text-left p-2 text-sm text-gray-400">
                          Price
                        </th>
                    ...
                    <tbody>
                      {mockTradingRecentTrades.map((trade, index) => (
                        <tr key={`${trade.time}-${index}`}>
                          <td
                            className={`text-left p-2 text-sm ${trade.side === "buy" ? "text-[#14F195]" : "text-red-500"}`}
                          >
        ...
        <Card className="bg-background border border-[#9945FF]/20 lg:col-span-1">
          ...
          <CardContent>
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between gap-2">
                  <Button
                    variant="outline"
                    size="sm"
                    className="flex-1 text-xs border-[#9945FF]/30"
                    onClick={() => handleAmountChange((0.25).toString())}
                  >
                    25%
                  </Button>
                  <Button
                    ...
                    size="sm"
                    className="flex-1 text-xs border-[#9945FF]/30"
                    onClick={() => handleAmountChange((0.5).toString())}
                  >
                    50%
                  </Button>
                  <Button
                    ...
                    size="sm"
                    className="flex-1 text-xs border-[#9945FF]/30"
                    onClick={() => handleAmountChange((0.75).toString())}
                  >
                    75%
                  </Button>
                  <Button
                    ...
                    size="sm"
                    className="flex-1 text-xs border-[#9945FF]/30"
                    onClick={() => handleAmountChange((1).toString())}
                  >
                    MAX
                  </Button>
                </div>
              ...
              <div className="space-y-2">
                <label htmlFor="total" className="text-sm text-gray-400">
                  Total (USDC)
                </label>
                <Input
                  id="total"
