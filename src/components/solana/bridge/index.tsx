...
function BridgeComponent({ demoMode = true }: { demoMode?: boolean }) {
  // Initialize with demo mode or connect to real wallet
  const { publicKey, connected } = demoMode ? { publicKey: new PublicKey("DUSTawucrTsGU8hcqRdHDCbuYhCPADMLM2VcCb8VnFnQ"), connected: true } : useWallet();
  const { connection } = demoMode ? { connection: null } : useConnection();
