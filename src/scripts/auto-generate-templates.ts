	...
	try {
		...
		for (const entry of templatesEntries) {
			if (entry.isDirectory()) {
				...
				const templateContent = fs.readFileSync(path.join(templatesPath, `${templateName}/${templateName}.tsx`), "utf-8");
    ...
    files: [
      {
        path: "${template.path}",
        name: "${template.name}",
        content: "${template.files?.find((file) => file.name === template.name)?.content}",
        type: "${template.type}",
      },
    ],
  }`,
...
// Main execution
console.log("🔄 Auto-generating templates.ts file...");
...
try {
	...
	console.log("✅ templates.ts file updated successfully!");
	console.log(`📁 Updated: ${templatesFilePath}`);
} catch (error) {
	console.error("❌ Error generating templates.ts:", error);
	process.exit(1);
}
