...
const BorrowContent = () => {
  ...
  const [borrowedAssets, setBorrowedAssets] = useState<{[key: string]: number}>({
    USDC: 250,
    SOL: 0.5,
  });
  ...
  useEffect(() => {
    if (collateralValue > 0) {
      const totalBorrowed = Object.entries(borrowedAssets).reduce((acc, [symbol, amount]) => {
  ...
  const handleBorrowAction = () => {
    if (!selectedAsset || !borrowAmount || Number.parseFloat(borrowAmount) <= 0) return;
    
    setBorrowedAssets(prev => ({
      ...prev,
      [selectedAsset]: (prev[selectedAsset] || 0) + Number.parseFloat(borrowAmount)
    }));
  ...
  const totalBorrowedValue = Object.entries(borrowedAssets).reduce((acc, [symbol, amount]) => {
    const asset = assets.find(a => a.symbol === symbol);
    return acc + (asset ? amount * asset.price : 0);
  }, 0);
  ...
  return (
    <Card className="w-[600px] bg-black text-white border border-[#9945FF]/20">
      ...
      <CardContent className="pt-6 space-y-6">
        <Tabs value={borrowTab} className="w-full" onValueChange={setBorrowTab}>
          <TabsList className="grid grid-cols-2 mb-4 bg-black border border-[#9945FF]/30">
            <TabsTrigger value="available" className="data-[state=active]:bg-[#9945FF]/20">
              Available Assets
            </TabsTrigger>
            <TabsTrigger value="your" className="data-[state=active]:bg-[#9945FF]/20">
              Your Borrows
            </TabsTrigger>
          </TabsList>
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="border-b border-[#9945FF]/20">
                      ...
                      <TableHead className="text-gray-400 text-right">APR</TableHead>
                      <TableHead className="text-gray-400 text-right">Available</TableHead>
                      <TableHead className="text-gray-400 text-right">Limit</TableHead>
                      <TableHead />
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {assets.map((asset) => {
                      ...
                      return (
                        <TableRow key={asset.symbol} className="border-b border-[#9945FF]/10">
                          <TableCell className="font-medium flex items-center gap-2">
                            <div className={`w-6 h-6 rounded-full ${asset.iconBg} flex items-center justify-center`}>
                              {asset.icon}
                            </div>
                            {asset.symbol}
                          </TableCell>
                          <TableCell className="text-right">{asset.apr}%</TableCell>
                          <TableCell className="text-right">{asset.available} {asset.symbol}</TableCell>
                          <TableCell className="text-right">${(asset.ltv * collateralValue / 100).toLocaleString(undefined, {maximumFractionDigits: 0})}</TableCell>
                          <TableCell className="text-right">
                            <Button
                              ...
                              onClick={() => {
                                ...
                                const element = document.getElementById('borrow-form');
            ...
            <div className="space-y-4">
              {isLoading ? (
                ...
                <Table>
                  ...
                  <TableBody>
                    {Object.entries(borrowedAssets).map(([symbol, amount]) => {
                      ...
                      return (
                        <TableRow key={symbol} className="border-b border-[#9945FF]/10">
                          <TableCell className="font-medium flex items-center gap-2">
                            <div className={`w-6 h-6 rounded-full ${asset.iconBg} flex items-center justify-center`}>
                              {asset.icon}
                            </div>
                          ...
                          <TableCell className="text-right">{amount.toLocaleString(undefined, {maximumFractionDigits: 4})}</TableCell>
                          <TableCell className="text-right">${(amount * asset.price).toLocaleString(undefined, {maximumFractionDigits: 2})}</TableCell>
                          <TableCell className="text-right">{asset.apr}%</TableCell>
                          <TableCell className="text-right">
                            <Button
                              size="sm"
                ...
                <div className="flex flex-col items-center justify-center py-8 text-center">
                  <p className="text-gray-400 mb-2">You have no borrowed assets</p>
                  <Button 
                    variant="outline"
                    className="border-[#9945FF] text-[#9945FF] hover:bg-[#9945FF]/10"
        ...
        <div id="borrow-form" className="bg-[#9945FF]/10 p-4 rounded-md mt-6">
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-between items-center">
              ...
              <span className="text-sm">${borrowValue.toLocaleString(undefined, {maximumFractionDigits: 2})}</span>
            </div>
            ...
            <div className="space-y-1">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className={ltv > 80 ? "text-red-500" : ltv > 65 ? "text-yellow-500" : "text-[#14F195]"}>
                  {ltv.toFixed(2)}%
                </span>
              </div>
            ...
            <Button 
              ...
              disabled={!connected || !selectedAsset || !borrowAmount || Number.parseFloat(borrowAmount) <= 0 || ltv >= 90}
              onClick={handleBorrowAction}
            >
              {!connected ? "Connect Wallet to Borrow" : 
               ltv >= 90 ? "Borrow Limit Exceeded" : 
      ...
      <CardFooter className="flex justify-between text-sm border-t border-[#9945FF]/20 pt-4">
        ...
        <div className="font-medium">${totalBorrowedValue.toLocaleString(undefined, {maximumFractionDigits: 2})}</div>
      </CardFooter>
    </Card>
  );
};
