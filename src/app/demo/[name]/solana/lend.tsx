...
const LendContent = () => {
  ...
  const [suppliedAssets, setSuppliedAssets] = useState<{[key: string]: number}>({
    USDC: 100,
    SOL: 0.75,
  });
  ...
  const handleSupplyAction = () => {
    if (!selectedAsset || !supplyAmount || Number.parseFloat(supplyAmount) <= 0) return;
    if (Number.parseFloat(supplyAmount) > walletBalance.amount) return;
    
    setSuppliedAssets(prev => ({
      ...prev,
      [selectedAsset]: (prev[selectedAsset] || 0) + Number.parseFloat(supplyAmount)
    }));
  ...
  const totalSuppliedValue = Object.entries(suppliedAssets).reduce((acc, [symbol, amount]) => {
  ...
  return (
    <Card className="w-[600px] bg-black text-white border border-[#14F195]/20">
      ...
      <CardContent className="pt-6 space-y-6">
        <Tabs value={lendTab} className="w-full" onValueChange={setLendTab}>
          <TabsList className="grid grid-cols-2 mb-4 bg-black border border-[#14F195]/30">
            <TabsTrigger value="available" className="data-[state=active]:bg-[#14F195]/20">
              Available Assets
            </TabsTrigger>
            <TabsTrigger value="your" className="data-[state=active]:bg-[#14F195]/20">
              Your Supply
            </TabsTrigger>
          </TabsList>
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="border-b border-[#14F195]/20">
                      ...
                      <TableHead className="text-gray-400 text-right">Wallet Balance</TableHead>
                      <TableHead />
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {assets.map((asset) => {
                      ...
                      return (
                        <TableRow key={asset.symbol} className="border-b border-[#14F195]/10">
                          ...
                          <TableCell className="text-right">{asset.totalSupply} {asset.symbol}</TableCell>
                          <TableCell className="text-right">{asset.walletBalance.toLocaleString(undefined, {maximumFractionDigits: 4})} {asset.symbol}</TableCell>
                          <TableCell className="text-right">
                            <Button
                              ...
                              onClick={() => {
                                ...
                                const element = document.getElementById('supply-form');
            ...
            <div className="space-y-4">
              {isLoading ? (
                ...
                <Table>
                  ...
                  <TableBody>
                    {Object.entries(suppliedAssets).map(([symbol, amount]) => {
                      ...
                      return (
                        <TableRow key={symbol} className="border-b border-[#14F195]/10">
                          <TableCell className="font-medium flex items-center gap-2">
                            <div className={`w-6 h-6 rounded-full ${asset.iconBg} flex items-center justify-center`}>
                              {asset.icon}
                            </div>
                ...
                <div className="flex flex-col items-center justify-center py-8 text-center">
                  <p className="text-gray-400 mb-2">You have no supplied assets</p>
                  <Button 
                    variant="outline"
                    className="border-[#14F195] text-[#14F195] hover:bg-[#14F195]/10"
        ...
        <div id="supply-form" className="bg-[#14F195]/10 p-4 rounded-md mt-6">
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-between items-center">
              ...
              <span className="text-sm">${supplyValue.toLocaleString(undefined, {maximumFractionDigits: 2})}</span>
            </div>
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                ...
                <span>
                  {selectedAsset ? (
                    <>
                      {walletBalance.amount.toLocaleString(undefined, {maximumFractionDigits: 4})} {selectedAsset} 
                      (${walletBalance.value.toLocaleString(undefined, {maximumFractionDigits: 2})})
                    </>
                  ) : "0.00"}
                </span>
              </div>
            ...
            <Button 
              ...
              disabled={!connected || !selectedAsset || !supplyAmount || 
                Number.parseFloat(supplyAmount) <= 0 || 
                Number.parseFloat(supplyAmount) > walletBalance.amount}
              onClick={handleSupplyAction}
            >
              {!connected ? "Connect Wallet to Supply" : 
               !selectedAsset ? "Select an Asset" :
               Number.parseFloat(supplyAmount) > walletBalance.amount ? "Insufficient Balance" : 
      ...
      <CardFooter className="flex justify-between text-sm border-t border-[#14F195]/20 pt-4">
        ...
        <div className="font-medium">${totalSuppliedValue.toLocaleString(undefined, {maximumFractionDigits: 2})}</div>
      </CardFooter>
    </Card>
  );
};
