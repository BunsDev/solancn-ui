...
export const transformTsxToJsx = async (tsxCode: string): Promise<string> => {
	try {
		...
		traverse(ast, {
			...
			ClassProperty(path) {
				...
				if ("accessibility" in path.node) {
					delete (path.node as unknown as Record<string, unknown>)
						.accessibility;
				}
			...
			JSXElement(path) {
				...
				if (
					...
					"typeParameters" in path.node.openingElement
				) {
					delete (
						path.node.openingElement as unknown as Record<string, unknown>
					).typeParameters;
				}
			},
		...
		console.warn(
			"AST transformation failed, falling back to regex-based transformation:",
			error,
		);
