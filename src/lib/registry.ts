...
import type { Style } from "@/components/solancn/registry/registry-styles";
// Import the registry index
import * as DefaultRegistry from "./registry/default";
import * as NewYorkRegistry from "./registry/new-york";
import type { ComponentEntry, Registry, StyleRegistry } from "./registry-types";
...
export function getFileTarget(filePath: string, fileType: string): string {
	...
	if (
		fileType.includes("component") ||
		fileType.includes("block") ||
		fileType.includes("example")
	) {
...
): Array<{ path: string; type: string; target: string }> {
