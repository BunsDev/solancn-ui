...
dev
  ...
  .action(async (options) => {
    ...
    if (!await fs.pathExists(pkgJsonPath)) {
      console.log(chalk.red('Error: No package.json found in the target directory.'));
      console.log(`Make sure you're running the command in a valid Next.js project directory.`);
      return;
    }
    ...
    const runCommand = (cmd: string, args: string[], options?: SpawnOptions) => {
  ...
  .action(async (options) => {
    ...
    try {
      ...
      if (options.init) {
        ...
        const initCommand = pkgManager === 'npm' ? 'npx' : pkgManager === 'yarn' ? 'yarn' : 'pnpm';
        const initArgs = pkgManager === 'npm' ? 
          ['storybook', 'init'] : 
          pkgManager === 'yarn' ? 
            ['dlx', 'storybook', 'init'] : 
            ['dlx', 'storybook', 'init'];
...
`;
          ...
          await fs.writeFile(path.join(storiesDir, 'Button.stories.js'), sampleStory);
      ...
      if (error instanceof Error && (error.message.includes('command not found') || error.message.includes('not recognized'))) {
        console.log('\nIt looks like Storybook might not be installed.');
        console.log(`Try running ${chalk.cyan('solancn dev storybook --init')} first to set up Storybook.`);
      }
    }
  });
...
`;
    ...
    try {
      ...
      if (componentType === 'ui') {
        console.log(chalk.bold('\nNext steps:'));
        console.log(`1. Add your component to the registry in ${chalk.cyan('registry.json')}`);
        console.log(`2. Create a documentation page in ${chalk.cyan(`src/app/docs/ui/${name.toLowerCase()}/page.tsx`)}`);
