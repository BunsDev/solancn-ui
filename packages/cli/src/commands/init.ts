...
export async function init() {
  ...
  try {
    ...
    try {
      ...
      console.log(`Registry fetch failed: ${error instanceof Error ? error.message : String(error)}`);
    }
    ...
    if (templateFound) {
      ...
      throw new Error(`Template "${answers.demos}" not found in registry or local templates`);
    }
  } catch (error) {
