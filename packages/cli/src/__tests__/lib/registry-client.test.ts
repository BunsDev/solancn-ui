...
describe('registry-client', () => {
  ...
  describe('Core Registry Functions', () => {
    ...
    it('should fetch items by type and filter them correctly', async () => {
      ...
      // Test components
      const components = await registryClient.fetchRegistryItems({ type: 'registry:component' });
      expect(components).toEqual(mockRegistryData?.components);

      // Test blocks
      const blocks = await registryClient.fetchRegistryItems({ type: 'registry:block' });
    ...
    it('should fetch a specific registry item by name and type', async () => {
      ...
      vi.spyOn(registryClient, 'fetchRegistryItem').mockImplementation(async (options) => {
