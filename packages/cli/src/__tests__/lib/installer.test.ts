...
describe('installer', () => {
  ...
  describe('installItem', () => {
    ...
    it('should install component successfully', async () => {
      vi.mocked(registryClient.fetchRegistryItem).mockResolvedValueOnce(mockComponent);
      ...
      const result = await installer.installItem('button', '/project/components', {
        ...options,
        itemType: 'component'
      });

      expect(logger.spinner).toHaveBeenCalledWith('Installing component: button');
      expect(registryClient.fetchRegistryItem).toHaveBeenCalledWith('button', 'component');
      expect(fs.ensureDir).toHaveBeenCalled();
      expect(fs.writeFile).toHaveBeenCalledTimes(mockComponent.files ? Object.keys(mockComponent.files).length : 0);
    ...
    it('should install block successfully', async () => {
      vi.mocked(registryClient.fetchRegistryItem).mockResolvedValueOnce(mockBlock);
      ...
      expect(registryClient.fetchRegistryItem).toHaveBeenCalledWith('hero', 'block');
      expect(fs.ensureDir).toHaveBeenCalled();
      expect(fs.writeFile).toHaveBeenCalledTimes(Object.keys(mockBlock.files || {}).length);
      expect(mockSpinner.succeed).toHaveBeenCalled();
      expect(result.success).toBe(true);
    ...
    it('should handle item not found', async () => {
      ...
      const result = await installer.installItem('nonexistent', '/project/components', options);
    ...
    it('should handle fs operation errors', async () => {
      vi.mocked(registryClient.fetchRegistryItem).mockResolvedValueOnce(mockComponent);
      vi.mocked(fs.ensureDir).mockRejectedValueOnce(new Error('Directory creation failed'));
      ...
      await expect(installer.installItem('button', '/project/components', options))
    ...
    it('should not install if files exist and force is false', async () => {
      ...
      const result = await installer.installItem('button', '/project/components', options);
  ...
  describe('uninstallItem', () => {
    ...
    it('should uninstall component successfully', async () => {
      ...
      const result = await installer.uninstallItem('button', '/project/components', {
        itemType: 'component'
      });

      expect(logger.spinner).toHaveBeenCalledWith('Uninstalling component: button');
      expect(registryClient.fetchRegistryItem).toHaveBeenCalledWith('button', 'component');
      expect(fs.remove).toHaveBeenCalledTimes(mockComponent.files ? Object.keys(mockComponent.files).length : 0);
      expect(mockSpinner.succeed).toHaveBeenCalled();
      expect(result.success).toBe(true);
    });
    ...
    it('should uninstall block successfully', async () => {
      ...
      expect(registryClient.fetchRegistryItem).toHaveBeenCalledWith('hero', 'block');
      expect(fs.remove).toHaveBeenCalledTimes(mockBlock.files ? Object.keys(mockBlock.files).length : 0);
      expect(mockSpinner.succeed).toHaveBeenCalled();
      expect(result.success).toBe(true);
    });
    ...
    it('should handle item not found in registry', async () => {
      ...
      const result = await installer.uninstallItem('nonexistent', '/project/components');
    ...
    it('should handle uninstall errors', async () => {
      ...
      await expect(installer.uninstallItem('button', '/project/components'))
  ...
  describe('isItemInstalled', () => {
    it('should return true when all item files exist', async () => {
      ...
      const result = await installer.isItemInstalled('button', 'component', '/project/components');
      
      expect(result).toBe(true as any);
      expect(fs.pathExists).toHaveBeenCalledTimes(mockComponent.files ? Object.keys(mockComponent.files).length : 0);
    });

    it('should return false when item not found in registry', async () => {
      vi.mocked(registryClient.fetchRegistryItem).mockResolvedValueOnce(null);

      const result = await installer.isItemInstalled('nonexistent', 'component', '/project/components');
    ...
    it('should return false when any file is missing', async () => {
      vi.mocked(registryClient.fetchRegistryItem).mockResolvedValueOnce(mockComponent);
      vi.mocked(fs.pathExists).mockResolvedValueOnce(true as unknown as any).mockResolvedValueOnce(false as unknown as any);
  ...
  describe('backupFiles', () => {
    ...
    it('should handle errors during backup', async () => {
      ...
      await expect(installer.backupFiles(files, targetDir))
  ...
  describe('backward compatibility functions', () => {
    ...
    it('should call installItem from installComponent', async () => {
      await installer.installComponent('button', '/project/components', { dependencies: false });
    ...
    it('should call installItem from installBlock', async () => {
      await installer.installBlock('hero', '/project/blocks', { dependencies: false });
    ...
    it('should call uninstallItem from uninstallComponent', async () => {
      await installer.uninstallComponent('button', '/project/components', { force: true });
    ...
    it('should call uninstallItem from uninstallBlock', async () => {
      await installer.uninstallBlock('hero', '/project/blocks', { force: true });
