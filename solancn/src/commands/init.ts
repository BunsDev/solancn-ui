...
export const init = new Command()
  .name("init")
  .description("Initialize your project with shadcn config & solancn")
  ...
  .action(async (opts) => {
    try {
      ...
      logger.info(
        `${chalk.green(
          "Success!",
        )} Project initialization completed. You may now add both shadcn and solancn components.`,
      );
      logger.info("");
    } catch (error) {
      handleError(error);
...
export async function promptForConfig(
  ...
  const config = rawConfigSchema.parse({
    ...
    aliases: {
      ...
      solancn: `${options.components}/solancn`,
    },
  });
