...
vi.mock('../utils/transformers', () => ({
  transform: vi.fn().mockResolvedValue('export function Button() { return <button>Click me</button> }'),
}));
...
describe('Component addition functionality', () => {
  ...
  it('should handle registry fallback when primary registry fails', async () => {
    ...
    vi.mocked(registry.getRegistryIndexSolancn).mockRejectedValueOnce(new Error('Failed to fetch from SolancnUI registry'));
