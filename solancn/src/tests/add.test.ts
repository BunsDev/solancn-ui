...
describe("Component addition functionality", () => {
  ...
  beforeEach(() => {
    ...
    (registry.getItemTargetPath as any).mockResolvedValue(
      "/project/components/ui/button",
    );

    // Mock successful transform
    (transform.transform as any).mockResolvedValue(
      "export function Button() { return <button>Click me</button> }",
    );
  });
  ...
  it("should handle registry error gracefully", async () => {
    // Mock primary registry failure
    (registry.getRegistryIndexSolancn as any).mockRejectedValue(
    ...
    // Verify component writing was attempted
    expect(fs.promises.writeFile).toHaveBeenCalledTimes(0);
  });
